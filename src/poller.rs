...
fn poll_codex() -> Result<UsageData, PollError> {
    let remote_result = match read_codex_credentials() {
        ...
        None => {
            diagnose::log("Codex usage remote poll skipped: no Codex credentials found");
...
fn try_usage_endpoint(token: &str) -> Result<Option<UsageData>, PollError> {
    ...
    if let Some(bucket) = &response.five_hour {
        data.session = UsageSection::from_used_percent(
            bucket.utilization,
    ...
    if let Some(bucket) = &response.seven_day {
        data.weekly = UsageSection::from_used_percent(
            bucket.utilization,
            parse_iso8601(bucket.resets_at.as_deref()),
            UsageSource::Remote,
        );
...
fn parse_rate_limit_headers(response: &ureq::Response) -> UsageData {
    ...
    data.session = UsageSection::from_used_percent(
        ...
        unix_to_system_time(get_header_i64(
            response,
    ...
    if data.session.percentage == 0.0 && data.weekly.percentage == 0.0 {
        ...
        if data.session.resets_at.is_none() && overall_reset.is_some() {
            data.session = UsageSection::from_used_percent(
                ...
                unix_to_system_time(overall_reset),
                UsageSource::Remote,
            );
        }
    }
...
fn codex_section_from_window(window: &CodexRateLimitWindow) -> UsageSection {
    UsageSection::from_used_percent(
        window.used_percent,
        unix_to_system_time(Some(window.reset_at)),
        UsageSource::Remote,
    )
}
...
fn codex_sessions_path() -> Option<PathBuf> {
    if let Some(codex_home) = std::env::var_os("CODEX_HOME").map(PathBuf::from) {
        return Some(codex_home.join("sessions"));
    }
...
fn codex_candidate_from_session_record(
    ...
    Some(CodexSessionCandidate { timestamp, usage })
}

fn codex_usage_from_session_rate_limits(
...
fn codex_section_from_session_limit(
    ...
    let reset_time = codex_session_reset_time(limit, record_timestamp);

    if let Some(used_percent) = json_number(limit, "used_percent") {
        return Some(UsageSection::from_used_percent(
            used_percent,
            reset_time,
    ...
    json_number(limit, "remaining_percent").map(|remaining_percent| {
        UsageSection::from_remaining_percent(
            remaining_percent,
            reset_time,
            UsageSource::LocalSession,
        )
...
fn codex_session_reset_time(
    limit: &serde_json::Value,
    record_timestamp: SystemTime,
) -> Option<SystemTime> {
    json_number(limit, "resets_at")
        .or_else(|| json_number(limit, "reset_at"))
        .map(|value| value as i64)
        .and_then(|unix_secs| unix_to_system_time(Some(unix_secs)))
        .or_else(|| {
            json_number(limit, "resets_in_seconds").and_then(|seconds| {
                if seconds.is_sign_negative() {
                    None
                } else {
                    Some(record_timestamp + Duration::from_secs(seconds.round() as u64))
                }
            })
        })
}
...
fn antigravity_section_from_quota(quota: AntigravityQuotaInfo) -> Option<UsageSection> {
    ...
    Some(UsageSection::from_remaining_percent(
        remaining * 100.0,
...
pub fn format_detail_line(section: &UsageSection) -> String {
    ...
    parts.push(section.source.label().to_string());
...
fn format_reset_time(resets_at: Option<SystemTime>) -> Option<String> {
    let reset = resets_at?;
...
mod tests {
    ...
    fn unique_test_dir(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "codex-usage-taskbar-{name}-{}-{suffix}",
            std::process::id()
        ))
    }

    #[test]
    fn codex_usage_response_tracks_used_remaining_reset_and_source() {
        let response: CodexUsageResponse = serde_json::from_str(
            r#"{
                "rate_limit": {
                    "primary_window": { "used_percent": 24.0, "reset_at": 1893456000 },
                    "secondary_window": { "used_percent": 33.0, "reset_at": 1894060800 }
                }
            }"#,
        )
        ...
        assert_eq!(usage.session.used_percent, 24.0);
        assert_eq!(usage.session.remaining_percent, 76.0);
        assert_eq!(usage.session.source, UsageSource::Remote);
    ...
    #[test]
    fn local_codex_session_jsonl_fallback_uses_latest_token_count() {
        let root = unique_test_dir("jsonl");
        let nested = root.join("2026").join("07").join("03");
        fs::create_dir_all(&nested).expect("test dir should be created");
        let file = nested.join("rollout.jsonl");
        fs::write(
            &file,
            concat!(
                r#"{"type":"event_msg","timestamp":"2026-07-03T01:00:00Z","payload":{"type":"token_count","rate_limits":{"primary":{"used_percent":90.0,"resets_at":1893450000},"secondary":{"used_percent":10.0,"resets_at":1894060000}}}}"#,
                "\n",
                r#"{"type":"event_msg","timestamp":"2026-07-03T02:00:00Z","payload":{"type":"token_count","rate_limits":{"primary":{"used_percent":24.0,"resets_at":1893456000},"secondary":{"used_percent":33.0,"resets_at":1894060800}}}}"#,
                "\n"
            ),
        )
        ...
        let usage =
            read_latest_codex_session_usage_from_root(&root).expect("local usage should parse");

        assert_eq!(usage.session.used_percent, 24.0);
        assert_eq!(usage.session.remaining_percent, 76.0);
        assert_eq!(usage.session.source, UsageSource::LocalSession);
        assert_eq!(usage.weekly.used_percent, 33.0);
