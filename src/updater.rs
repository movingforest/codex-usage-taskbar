...
const RELEASE_ASSET_NAME: &str = "codex-usage-taskbar.exe";
...
const WINGET_PACKAGE_ID: &str = "Forest.CodexUsageTaskbar";
...
fn updates_dir() -> Result<PathBuf, String> {
    dirs::data_local_dir()
        .map(|dir| dir.join("CodexUsageTaskbar").join("updates"))
        .or_else(|| {
            Some(
                std::env::temp_dir()
